<div class="card-group chat-app">
    <div class="col-izq d-flex card people-list no-lista">
        <div class="input-group">

            <input type="text" class="form-control" placeholder="Buscar conversacion..." id="buscarklk">

        </div>
        
        <div id="bloque1" class="bloques">

            <ul class="list-unstyled chat-list mt-2 mb-3" id="chats">
                <li class="clearfix bordes active " id="chat_1114">
                    <div class="avatar iconos">
                        <i class="fa fa-user fa-lg" aria-hidden="true"></i>
                    </div>

                    <div class="about" role="row">
                        <div class="filaNombre" aria-colindex="2">
                            <div class="nombre">

                                <a href=""
                                    class="conversacion" data-mid="1114" data-sufijo="DE">RICARDO MARTINEZ </a>

                            </div>

                            <div class="ultimomsg">

                                Hoy 09:26 AM <a href="javascript:;" class="eliminarchat btn-sm" data-mid="1114"
                                    data-sufijo="DE" data-asunto="LINDO PERRITO CON MAS O en La Vega"
                                    data-nombre="RICARDO MARTINEZ" class="btn-sm"><i class="click fa fa-times"
                                        aria-hidden="true"></i></a>

                            </div>
                        </div>
                        <div class="filaAsunto" aria-colindex="2">
                            <div class="asunto">

                                LINDO PERRITO CON MAS O en La Vega
                            </div>
                            <div class="opciones">

                                <span id="msgnuevo_1114" class="msgnuevo badge badge-success"></span>
                            </div>
                        </div>
                    </div>
                </li>
                <li class="clearfix bordes  " id="chat_1055">
                    <div class="avatar iconos">
                        <i class="fa fa-user fa-lg" aria-hidden="true"></i>
                    </div>

                    <div class="about" role="row">
                        <div class="filaNombre" aria-colindex="2">
                            <div class="nombre">

                                <a href=""
                                    class="conversacion" data-mid="1055" data-sufijo="DE">RICARDO MARTINEZ </a>

                            </div>

                            <div class="ultimomsg">

                                04-08-2022 <a href="javascript:;" class="eliminarchat btn-sm" data-mid="1055"
                                    data-sufijo="DE" data-asunto="gomas 195/45/15 nuevas y usadas"
                                    data-nombre="RICARDO MARTINEZ" class="btn-sm"><i class="click fa fa-times"
                                        aria-hidden="true"></i></a>

                            </div>
                        </div>
                        <div class="filaAsunto" aria-colindex="2">
                            <div class="asunto">

                                gomas 195/45/15 nuevas y usadas
                            </div>
                            <div class="opciones">

                                <span id="msgnuevo_1055" class="msgnuevo badge badge-success"></span>
                            </div>
                        </div>
                    </div>
                </li>
                <li class="clearfix bordes  " id="chat_1055">
                    <div class="avatar iconos">
                        <i class="fa fa-user fa-lg" aria-hidden="true"></i>
                    </div>

                    <div class="about" role="row">
                        <div class="filaNombre" aria-colindex="2">
                            <div class="nombre">

                                <a href=""
                                    class="conversacion" data-mid="1055" data-sufijo="DE">RICARDO MARTINEZ </a>

                            </div>

                            <div class="ultimomsg">

                                04-08-2022 <a href="javascript:;" class="eliminarchat btn-sm" data-mid="1055"
                                    data-sufijo="DE" data-asunto="gomas 195/45/15 nuevas y usadas"
                                    data-nombre="RICARDO MARTINEZ" class="btn-sm"><i class="click fa fa-times"
                                        aria-hidden="true"></i></a>

                            </div>
                        </div>
                        <div class="filaAsunto" aria-colindex="2">
                            <div class="asunto">

                                gomas 195/45/15 nuevas y usadas
                            </div>
                            <div class="opciones">

                                <span id="msgnuevo_1055" class="msgnuevo badge badge-success"></span>
                            </div>
                        </div>
                    </div>
                </li>
                <li class="clearfix bordes  " id="chat_1055">
                    <div class="avatar iconos">
                        <i class="fa fa-user fa-lg" aria-hidden="true"></i>
                    </div>

                    <div class="about" role="row">
                        <div class="filaNombre" aria-colindex="2">
                            <div class="nombre">

                                <a href=""
                                    class="conversacion" data-mid="1055" data-sufijo="DE">RICARDO MARTINEZ </a>

                            </div>

                            <div class="ultimomsg">

                                04-08-2022 <a href="javascript:;" class="eliminarchat btn-sm" data-mid="1055"
                                    data-sufijo="DE" data-asunto="gomas 195/45/15 nuevas y usadas"
                                    data-nombre="RICARDO MARTINEZ" class="btn-sm"><i class="click fa fa-times"
                                        aria-hidden="true"></i></a>

                            </div>
                        </div>
                        <div class="filaAsunto" aria-colindex="2">
                            <div class="asunto">

                                gomas 195/45/15 nuevas y usadas
                            </div>
                            <div class="opciones">

                                <span id="msgnuevo_1055" class="msgnuevo badge badge-success"></span>
                            </div>
                        </div>
                    </div>
                </li>
                <li class="clearfix bordes  " id="chat_1055">
                    <div class="avatar iconos">
                        <i class="fa fa-user fa-lg" aria-hidden="true"></i>
                    </div>

                    <div class="about" role="row">
                        <div class="filaNombre" aria-colindex="2">
                            <div class="nombre">

                                <a href=""
                                    class="conversacion" data-mid="1055" data-sufijo="DE">RICARDO MARTINEZ </a>

                            </div>

                            <div class="ultimomsg">

                                04-08-2022 <a href="javascript:;" class="eliminarchat btn-sm" data-mid="1055"
                                    data-sufijo="DE" data-asunto="gomas 195/45/15 nuevas y usadas"
                                    data-nombre="RICARDO MARTINEZ" class="btn-sm"><i class="click fa fa-times"
                                        aria-hidden="true"></i></a>

                            </div>
                        </div>
                        <div class="filaAsunto" aria-colindex="2">
                            <div class="asunto">

                                gomas 195/45/15 nuevas y usadas
                            </div>
                            <div class="opciones">

                                <span id="msgnuevo_1055" class="msgnuevo badge badge-success"></span>
                            </div>
                        </div>
                    </div>
                </li>
                <li class="clearfix bordes  " id="chat_1055">
                    <div class="avatar iconos">
                        <i class="fa fa-user fa-lg" aria-hidden="true"></i>
                    </div>

                    <div class="about" role="row">
                        <div class="filaNombre" aria-colindex="2">
                            <div class="nombre">

                                <a href=""
                                    class="conversacion" data-mid="1055" data-sufijo="DE">RICARDO MARTINEZ </a>

                            </div>

                            <div class="ultimomsg">

                                04-08-2022 <a href="javascript:;" class="eliminarchat btn-sm" data-mid="1055"
                                    data-sufijo="DE" data-asunto="gomas 195/45/15 nuevas y usadas"
                                    data-nombre="RICARDO MARTINEZ" class="btn-sm"><i class="click fa fa-times"
                                        aria-hidden="true"></i></a>

                            </div>
                        </div>
                        <div class="filaAsunto" aria-colindex="2">
                            <div class="asunto">

                                gomas 195/45/15 nuevas y usadas
                            </div>
                            <div class="opciones">

                                <span id="msgnuevo_1055" class="msgnuevo badge badge-success"></span>
                            </div>
                        </div>
                    </div>
                </li>
                <li class="clearfix bordes  " id="chat_1055">
                    <div class="avatar iconos">
                        <i class="fa fa-user fa-lg" aria-hidden="true"></i>
                    </div>

                    <div class="about" role="row">
                        <div class="filaNombre" aria-colindex="2">
                            <div class="nombre">

                                <a href=""
                                    class="conversacion" data-mid="1055" data-sufijo="DE">RICARDO MARTINEZ </a>

                            </div>

                            <div class="ultimomsg">

                                04-08-2022 <a href="javascript:;" class="eliminarchat btn-sm" data-mid="1055"
                                    data-sufijo="DE" data-asunto="gomas 195/45/15 nuevas y usadas"
                                    data-nombre="RICARDO MARTINEZ" class="btn-sm"><i class="click fa fa-times"
                                        aria-hidden="true"></i></a>

                            </div>
                        </div>
                        <div class="filaAsunto" aria-colindex="2">
                            <div class="asunto">

                                gomas 195/45/15 nuevas y usadas
                            </div>
                            <div class="opciones">

                                <span id="msgnuevo_1055" class="msgnuevo badge badge-success"></span>
                            </div>
                        </div>
                    </div>
                </li>
                <li class="clearfix bordes  " id="chat_1055">
                    <div class="avatar iconos">
                        <i class="fa fa-user fa-lg" aria-hidden="true"></i>
                    </div>

                    <div class="about" role="row">
                        <div class="filaNombre" aria-colindex="2">
                            <div class="nombre">

                                <a href=""
                                    class="conversacion" data-mid="1055" data-sufijo="DE">RICARDO MARTINEZ </a>

                            </div>

                            <div class="ultimomsg">

                                04-08-2022 <a href="javascript:;" class="eliminarchat btn-sm" data-mid="1055"
                                    data-sufijo="DE" data-asunto="gomas 195/45/15 nuevas y usadas"
                                    data-nombre="RICARDO MARTINEZ" class="btn-sm"><i class="click fa fa-times"
                                        aria-hidden="true"></i></a>

                            </div>
                        </div>
                        <div class="filaAsunto" aria-colindex="2">
                            <div class="asunto">

                                gomas 195/45/15 nuevas y usadas
                            </div>
                            <div class="opciones">

                                <span id="msgnuevo_1055" class="msgnuevo badge badge-success"></span>
                            </div>
                        </div>
                    </div>
                </li>
                <li class="clearfix bordes  " id="chat_1055">
                    <div class="avatar iconos">
                        <i class="fa fa-user fa-lg" aria-hidden="true"></i>
                    </div>

                    <div class="about" role="row">
                        <div class="filaNombre" aria-colindex="2">
                            <div class="nombre">

                                <a href=""
                                    class="conversacion" data-mid="1055" data-sufijo="DE">RICARDO MARTINEZ </a>

                            </div>

                            <div class="ultimomsg">

                                04-08-2022 <a href="javascript:;" class="eliminarchat btn-sm" data-mid="1055"
                                    data-sufijo="DE" data-asunto="gomas 195/45/15 nuevas y usadas"
                                    data-nombre="RICARDO MARTINEZ" class="btn-sm"><i class="click fa fa-times"
                                        aria-hidden="true"></i></a>

                            </div>
                        </div>
                        <div class="filaAsunto" aria-colindex="2">
                            <div class="asunto">

                                gomas 195/45/15 nuevas y usadas
                            </div>
                            <div class="opciones">

                                <span id="msgnuevo_1055" class="msgnuevo badge badge-success"></span>
                            </div>
                        </div>
                    </div>
                </li>
                <li class="clearfix bordes  " id="chat_1055">
                    <div class="avatar iconos">
                        <i class="fa fa-user fa-lg" aria-hidden="true"></i>
                    </div>

                    <div class="about" role="row">
                        <div class="filaNombre" aria-colindex="2">
                            <div class="nombre">

                                <a href=""
                                    class="conversacion" data-mid="1055" data-sufijo="DE">RICARDO MARTINEZ </a>

                            </div>

                            <div class="ultimomsg">

                                04-08-2022 <a href="javascript:;" class="eliminarchat btn-sm" data-mid="1055"
                                    data-sufijo="DE" data-asunto="gomas 195/45/15 nuevas y usadas"
                                    data-nombre="RICARDO MARTINEZ" class="btn-sm"><i class="click fa fa-times"
                                        aria-hidden="true"></i></a>

                            </div>
                        </div>
                        <div class="filaAsunto" aria-colindex="2">
                            <div class="asunto">

                                gomas 195/45/15 nuevas y usadas
                            </div>
                            <div class="opciones">

                                <span id="msgnuevo_1055" class="msgnuevo badge badge-success"></span>
                            </div>
                        </div>
                    </div>
                </li>
                <li class="clearfix bordes  " id="chat_8">
                    <div class="avatar iconos">
                        <i class="fa fa-user fa-lg" aria-hidden="true"></i>
                    </div>

                    <div class="about" role="row">
                        <div class="filaNombre" aria-colindex="2">
                            <div class="nombre">

                                <a href="/klk/8/YTo2OntzOjI6IklEIjtzOjE6IjgiO3M6MTA6Ik5PTUJSRUNIQVQiO3M6MTY6IlJJQ0FSRE8gTUFSVElORVoiO3M6MTE6IkFSVElDVUxPX0lEIjtzOjc6IjY3OTQwNzMiO3M6NjoiQVNVTlRPIjtzOjE2OiJUb3lvdGEgVml0eiAyMDAzIjtzOjI6IkRFIjtzOjE6IjMiO3M6NDoiUEFSQSI7czoyOiIzNiI7fQ=="
                                    class="conversacion" data-mid="8" data-sufijo="DE">RICARDO MARTINEZ </a>

                            </div>

                            <div class="ultimomsg">

                                04-08-2022 <a href="javascript:;" class="eliminarchat" data-mid="8" data-sufijo="DE"
                                    data-asunto="Toyota Vitz 2003" data-nombre="RICARDO MARTINEZ" class="btn-sm"><i
                                        class="fa fa-times" aria-hidden="true"></i></a>

                            </div>
                        </div>
                        <div class="filaAsunto" aria-colindex="2">
                            <div class="asunto">

                                Toyota Vitz 2003
                            </div>
                            <div class="opciones">

                                <span id="msgnuevo_8" class="msgnuevo badge badge-success"></span>
                            </div>
                        </div>
                    </div>
                </li>

            </ul>

        </div>
    </div>

    <script type="text/javascript">
        var mid = 1114
    </script>


    <div class="chat col-der d-flex card no-chat1">
        <div class="chat-header clearfix">
            <div class="card-title cabecera">
                <a href="/" data-toggle="modal" data-target="#view_info">
                    <img src="https://cdn.computerhoy.com/sites/navi.axelspringer.es/public/styles/1200/public/media/image/2022/06/shark-suv-jeep-electrico-ruedas-cuadradas-2743927.jpg?itok=EhrMlar1" alt="">
                </a>
                <div class="card-title">
                    <div class="m-b-0">RICARDO MARTINEZ

                        <a type="button" class="menu btn btn-light btn-sm float-right fixed-start" aria-hidden="true"
                            data-toggle="dropdown" aria-haspopup="false" aria-expanded="false"><i
                                class="fa fa-bars"></i>
                        </a><br>

                        <div class="dropdown-menu bordes">
                            <a class="dropdown-item" href="#">Eliminar</a>
                            <a id="reportar" class="dropdown-item" href="#">Reportar</a>
                        </div>

                        <a id="asunto" href="/asunto_6794073.html">LINDO PERRITO CON MAS O en La Vega</a>
                    </div>
                    <small class="text-success bold" id="visto_1114">Escribiendo... </small>
                </div>
            </div>
        </div>

        <div id="bloque2" class="bloques card-body chat-history">

            <ul class="m-b-0" id="conversacion">

                <li class="clearfix">
                    <div class="message-data text-right">
                        <span class="message-data-time">03-08-2022 08:06 PM</span>
                    </div>
                    <div class="message  my-message float-right ">a ver com oesto inicia</div>
                </li>
                <li class="clearfix">
                    <div class="message-data ">
                        <span class="message-data-time">03-08-2022 08:25 PM</span>
                    </div>
                    <div class="message  other-message ">salio bien la imagen</div>
                </li>
                <li class="clearfix">
                    <div class="message-data text-right">
                        <span class="message-data-time">04-08-2022 09:33 AM</span>
                    </div>
                    <div class="message  my-message float-right ">&#x1f7ea</div>
                </li>
                <li class="clearfix">
                    <div class="message-data text-right">
                        <span class="message-data-time">04-08-2022 09:33 AM</span>
                    </div>
                    <div class="message  my-message float-right ">&#x1f7ea</div>
                </li>
                <li class="clearfix">
                    <div class="message-data text-right">
                        <span class="message-data-time">04-08-2022 09:34 AM</span>
                    </div>
                    <div class="message  my-message float-right ">&#x1f192;</div>
                </li>
                <li class="clearfix">
                    <div class="message-data text-right">
                        <span class="message-data-time">04-08-2022 09:35 AM</span>
                    </div>
                    <div class="message  my-message float-right ">&#x270c;&#xfe0f;</div>
                </li>
                <li class="clearfix">
                    <div class="message-data text-right">
                        <span class="message-data-time">04-08-2022 09:36 AM</span>
                    </div>
                    <div class="message  my-message float-right ">&#x1f1ed; &#x1f1f4; &#x1f1f1; &#x1f1e6;</div>
                </li>
                <li class="clearfix">
                    <div class="message-data text-right">
                        <span class="message-data-time">04-08-2022 11:06 AM</span>
                    </div>
                    <div class="message  my-message float-right ">como tamo</div>
                </li>
                <li class="clearfix">
                    <div class="message-data ">
                        <span class="message-data-time">04-08-2022 02:45 PM</span>
                    </div>
                    <div class="message  other-message ">que mas tenemos?</div>
                </li>
                <li class="clearfix">
                    <div class="message-data ">
                        <span class="message-data-time">04-08-2022 02:51 PM</span>
                    </div>
                    <div class="message  other-message ">nada por ahoar</div>
                </li>
                <li class="clearfix">
                    <div class="message-data ">
                        <span class="message-data-time">04-08-2022 02:57 PM</span>
                    </div>
                    <div class="message  other-message ">ta bien enteonces</div>
                </li>
                <li class="clearfix">
                    <div class="message-data text-right">
                        <span class="message-data-time">04-08-2022 02:58 PM</span>
                    </div>
                    <div class="message  my-message float-right ">bien todo</div>
                </li>
                <li class="clearfix">
                    <div class="message-data ">
                        <span class="message-data-time">Hoy 09:24 AM</span>
                    </div>
                    <div class="message  other-message ">hola</div>
                </li>
                <li class="clearfix">
                    <div class="message-data ">
                        <span class="message-data-time">Hoy 09:24 AM</span>
                    </div>
                    <div class="message  other-message ">buenas tardes</div>
                </li>
                <li class="clearfix">
                    <div class="message-data text-right">
                        <span class="message-data-time">Hoy 09:25 AM</span>
                    </div>
                    <div class="message  my-message float-right ">ya te borre</div>
                </li>
                <li class="clearfix">
                    <div class="message-data ">
                        <span class="message-data-time">Hoy 09:26 AM</span>
                    </div>
                    <div class="message  other-message ">hola </div>
                </li>
            </ul>
        </div>
        <div class="chat-message card-footer clearfix">
            <form nombre="frmklkmsgr" id="frmklkmsgr">
                <div class="input-group mb-0">
                    <input type="text" nombre="klkmsg" id="klkmsg" class="form-control" data-mid="1114"
                    data-sufijo="PARA" placeholder="Escriba su mensaje aqui...">
                    <button nombre="Enviar" type="submit" id="Enviar" data-usu="3" data-mid="1114" data-hora="10:15 AM"
                    data-sufijo="PARA" class="btn btn-success" disabled><i class="fa fa-send"></i></button>
                </div>
            </form>
        </div>
        
    </div>
</div>